...
import { knockoutPointsForMatch, type KOPickGrade } from "@/lib/scoring";
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex flex-1 flex-col justify-around gap-3">
        {matches.map((no) => (
          <MatchBox key={no} m={resolved.get(no)!} onPick={onPick} align={align} gradePick={gradePick} />
        ))}
      </div>
    </div>
  );
...
export function BracketTree({ resolved, onPick, gradePick }: BracketTreeProps) {
  ...
  return (
    <div className="overflow-x-auto pb-4">
      <div className="flex min-h-[760px] min-w-max items-stretch gap-4">
        <Column label={ROUND_LABEL.R32} matches={L.R32} resolved={resolved} onPick={onPick} gradePick={gradePick} />
        <Column label={ROUND_LABEL.R16} matches={L.R16} resolved={resolved} onPick={onPick} gradePick={gradePick} />
        <Column label={ROUND_LABEL.QF} matches={L.QF} resolved={resolved} onPick={onPick} gradePick={gradePick} />
        <Column label={ROUND_LABEL.SF} matches={L.SF} resolved={resolved} onPick={onPick} gradePick={gradePick} />

        <div className="flex flex-col items-center justify-center px-2">
          <div className="mb-2 text-center text-[10px] font-bold uppercase tracking-wide text-[var(--wc-accent)]">
            Final
          </div>
          <MatchBox m={finalM} onPick={onPick} gradePick={gradePick} />
          ...
          <div className="mt-4 w-44">
            ...
            <MatchBox m={thirdM} onPick={onPick} gradePick={gradePick} />
          </div>
        </div>

        <Column label={ROUND_LABEL.SF} matches={R.SF} resolved={resolved} onPick={onPick} align="right" gradePick={gradePick} />
        <Column label={ROUND_LABEL.QF} matches={R.QF} resolved={resolved} onPick={onPick} align="right" gradePick={gradePick} />
        <Column label={ROUND_LABEL.R16} matches={R.R16} resolved={resolved} onPick={onPick} align="right" gradePick={gradePick} />
        <Column label={ROUND_LABEL.R32} matches={R.R32} resolved={resolved} onPick={onPick} align="right" gradePick={gradePick} />
      </div>
    </div>
  );
}
