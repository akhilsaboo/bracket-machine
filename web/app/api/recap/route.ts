...
// Personalized Matchday-1 recap to EVERY signed-up email (minus opt-outs). Same
// safety model as /api/broadcast: admin/cron-gated, ?dry=1 previews, refuses to
// send before RECAP_SEND_AT_ISO, idempotent via email_reminders_log (recap-md1-v1),
// ?force=1 overrides, ?to=<email> sends one test copy. Cron-fired at 8am PT Jun 18.
// All per-user numbers come from the global leaderboard snapshot — no re-scoring.
...
export async function GET(req: Request) {
  ...
  const due = force || Date.now() >= Date.parse(RECAP_SEND_AT_ISO);

  const { data: log } = await sb
    .from("email_reminders_log")
    .select("milestone_key")
    .eq("milestone_key", RECAP_KEY)
    .maybeSingle();
  ...
  if (!snap.hasResults || !snap.rows?.length) {
    return Response.json({ error: "no results yet — nothing to recap" }, { status: 409 });
  }
  ...
  const ctx: Ctx = { userBest, bracketPoints, scores: snap.scores ?? [], total: snap.totalEntries, names, userPools };
  ...
  if (to) {
    ...
    const ok = await sendEmail(to, recapSubject(data), recapHtml(data));
  ...
  if (dry) {
    ...
    return Response.json({
      dryRun: true,
      scheduledFor: RECAP_SEND_AT_ISO,
      due,
  ...
  if (!due) return Response.json({ scheduled: RECAP_SEND_AT_ISO, note: "not due yet", recipients: people.length });
  ...
  const { sent, failed } = await sendBatch(
    people.map((p) => {
      const d = buildData(p.id, p.email, ctx);
      return { to: p.email, subject: recapSubject(d), html: recapHtml(d) };
    }),
  );
  await sb.from("email_reminders_log").upsert({ milestone_key: RECAP_KEY }, { onConflict: "milestone_key", ignoreDuplicates: true });
